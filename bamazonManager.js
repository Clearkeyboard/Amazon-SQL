...
var addInventory = function(){
	...
	prompt.get(addInvt, function(err, res){
		...
		connection.query("UPDATE products SET stock_quantity = (stock_quantity + ?) WHERE id = ?;", [inventoryUpdate[0].inventoryAmount, inventoryUpdate[0].inventoryID], function(err, result){

			if(err) console.log('error '+ err);

			connection.query("SELECT * FROM products WHERE id = ?", inventoryUpdate[0].inventoryID, function(error, resOne){
				console.log('');
...
var addNewProduct = function(){
	...
	prompt.get(newProduct, function(err, res){
		...
		connection.query('INSERT INTO Products (id, product_name, department_name, price, stock_quantity) VALUES (?, ?, ?, ?, ?);', [addedProduct[0].newIdNum, addedProduct[0].newItemName, addedProduct[0].newItemDepartment, addedProduct[0].newItemPrice, addedProduct[0].newStockQuantity], function(err, result){
